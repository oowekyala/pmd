...
import net.sourceforge.pmd.lang.rule.xpath.impl.XPathFunctionDefinition.Type;
    ...
    public ExtensionFunctionCall makeCallExpression() {
        ...
        return new ExtensionFunctionCall() {
            ...
            public Sequence call(XPathContext context, Sequence[] arguments) throws XPathException {
                ...
                return convertToSaxon(result, definition.getResultType(), context);
            }
        };
    }

    private static Sequence convertToSaxon(Object result, Type type, XPathContext context) throws ValidationException {

        switch (type) {
        case SINGLE_BOOLEAN:
            return BooleanValue.get((Boolean) result);
        case SINGLE_INTEGER:
            return Int64Value.makeIntegerValue((Integer) result);
        case SINGLE_STRING:
            return new StringValue((String) result);
        case OPTIONAL_STRING:
            return result instanceof Optional && ((Optional<String>) result).isPresent()
        ...
        case OPTIONAL_DECIMAL:
            return result instanceof Optional && ((Optional<Double>) result).isPresent()
                   ? new BigDecimalValue(((Optional<Double>) result).get())
                   : EmptySequence.getInstance();
        case NULLABLE_ELEMENT:
            if (result instanceof Node) {
                Item contextItem = context.getContextItem();
                if (!(contextItem instanceof AstElementNode)) {
                    throw new IllegalStateException("Cannot convert nodes without a context item");
                }
                return ((AstElementNode) contextItem).getTreeInfo().findWrapperFor((Node) result);
            }
        ...
        case ELEMENT_SEQUENCE:
            if (result instanceof List) {
                ...
                AstTreeInfo treeInfo = ((AstElementNode) contextItem).getTreeInfo();
                List<AstElementNode> usages = CollectionUtil.map(
                    (List<Node>) result,
                    treeInfo::findWrapperFor
                );
                return new SequenceExtent.Of<>(usages);
            }
            return EmptySequence.getInstance();

        default:
            throw new UnsupportedOperationException("Don't know how to convert result type " + type);
        }
    }
}
