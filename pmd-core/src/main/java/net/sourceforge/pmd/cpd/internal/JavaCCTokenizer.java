...
public abstract class JavaCCTokenizer implements Tokenizer {
    ...
    public void tokenize(TextDocument textDocument, Tokens tokenEntries) throws IOException {
        TokenManager<JavaccToken> tokenManager = getLexerForSource(textDocument);
        try {
            ...
            while (currentToken != null) {
                tokenEntries.add(processToken(tokenEntries, currentToken, textDocument.getDisplayName()));
                currentToken = tokenFilter.getNextToken();
            }
        } catch (TokenMgrError e) {
            throw e.setFileName(textDocument.getDisplayName());
        } finally {
